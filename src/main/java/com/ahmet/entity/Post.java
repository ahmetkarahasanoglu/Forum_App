...
public class Post {
    ...
    @ManyToOne(fetch = FetchType.EAGER) // Eager: brings the 'user' right away ||| Lazy: When we retrieve a post, its user will not be retrieved immediately (until it is explicitly accessed for the first time -> post.getUser() . ||| [ilişkileri anlamak için örnek durum: Bir okulun bir adresi olabilir (one-to-one). Bir okulun birçok öğretmeni olabilir (one-to-many). Birçok öğretmenin bir okulu vardır (many-to-one). Birçok öğretmenin birçok öğrencisi ve aynı zamanda birçok öğrencinin birçok öğretmeni olabilir (many-to-many)]
...
//    @JsonIgnore // Ignore the 'user' field when converting an object to JSON and a JSON to an object (serialization & deserialization).
